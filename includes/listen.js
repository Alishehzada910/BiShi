module.exports = function ({ api, models }) {
    ...
    const checkttDataPath = __dirname + '/../Shehzada/commands/checktuongtac/';
    setInterval(async () => {
      ...
      try {
        if (day != day_now) {
            ...
            console.log('Shehzada');
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    const checktt = JSON.parse(fs.readFileSync(checkttDataPath + checkttFile));
                    let storage = [], count = 1;
                    for (const item of checktt.day) {
                        const userName = await Users.getNameUser(item.id) || 'Shehzada';
                ...
                if (day_now == 1) {
                    ...
                    for (const checkttFile of checkttData) {
                        ...
                        for (const item of checktt.week) {
                            const userName = await Users.getNameUser(item.id) || 'Shehzada Hun Yar';
                            const itemToPush = item;
                            itemToPush.name = userName;
                            storage.push(itemToPush);
                        };
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ Shehzada ]');
            ...
            logger.loader(global.getText('listen', 'loadedEnvironmentUser')), logger(global.getText('listen', 'successLoadEnvironment'), '[ Shehzada ]');
        } catch (error) {
    ...
    const checkAndExecuteEvent = async () => {
        ...
        //GET CURRENT TIME
        var timeVN = moment().tz('Asia/Pakistan').format('DD/MM/YYYY_HH:mm:ss');
        ...
        for (el of temp) {
            ...
            if ("ATTACHMENT" in el) {
                ...
                for (a of el.ATTACHMENT) {
                    ...
                    fs.writeFileSync(__dirname + `/../Shehzada/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
